...
<template>
  <!-- side nav -->
  <div class="lg:w-[25%] border-r border-td_seprator px-2 lg:px-3 lg:pr-8 py-2 flex flex-col justify-between">
    ...
    <div>
      ...
      <div class="mt-8 text-left">
        <button v-for="tab in tabs" :key="tab.id" @click="id = tab.id" :class="`focus:outline-none hover:text-td_blue py-2 px-6
        ...
        ${id === tab.id ? 'text-td_dark_blue' : 'text-td_xlt_grey'}`">
          ...
          <p :class="`text-lg font-ChripRegular text-left hidden lg:block ${id === tab.id ? 'font-ChripBold' : ''}`">
            {{ tab.title }} </p>
        </button>
      </div>
